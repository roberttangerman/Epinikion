	...
	class Porto_Infinite_Scroll {
		...
		private function check_if_infinite() {
			...
			if ( is_archive() || is_category() || is_home() ) {
				...
				if ( ( 'post' == $post_type || 'portfolio' == $post_type || 'member' == $post_type ) && ( $term = get_queried_object() ) && isset( $term->term_id ) ) {
					...
					$is_infinite  = $term_options ? ( get_metadata( $term->taxonomy, $term->term_id, $post_type . '_infinite', true ) != $post_type . '_infinite' ? true : false ) : ( $porto_settings[ $global_setting_name ] && 'ajax' != $porto_settings[ $global_setting_name ] ? $porto_settings[ $global_setting_name ] : false );
				} elseif ( $post_type ) {
		...
		public function add_scripts() {
			...
			if ( $this->is_infinite ) {
				...
				wp_register_script( 'jquery-infinite-scroll', PORTO_URI . '/js/libs/jquery.infinite-scroll.min.js', array(), '2.1.0', true );
				wp_register_script( 'porto-infinite-scroll', PORTO_LIB_URI . '/infinite-scroll/infinite-scroll.min.js', array(), PORTO_VERSION, true );
			}
		}
	}
